...
/* ------------------------------------------------------------------ */
/*  Local overdue calculator for classroom context                      */
/* ------------------------------------------------------------------ */
...
function calcClassroomOverdue(
  ...
  currentMonth: string,
  currentYear: number
): OverdueCalcResult | null {
  // Check if there's a pending payment for current month
  let hasPendingPayment = false;
  let pendingPaymentId: string | null = null;
  const currentMonthPending = payments.find(
    (p) => p.month === currentMonth && p.year === currentYear && p.remainingAmount > 0
  );
  ...
  // ── Day-level check: if next due date hasn't arrived yet, skip ──
  ...
  const overduePayments: OverdueCalcResult['overduePayments'] = [];
...
export async function GET(
  ...
  try {
    ...
    // 1. Get the classroom with its schedules, levels, students, and enrollments
    ...
    const studentMap = new Map<string, { student: any; levelName: string; subjectName: string }>();
    ...
    // 3. Batch-fetch enrollment-linked payments for students that have enrollments
    const allEnrollmentIds = Array.from(studentMap.values())
      .filter(({ student }) => student.enrollments.length > 0)
      .flatMap(({ student }) => student.enrollments.map((e) => e.id));

    const allEnrollmentPayments = allEnrollmentIds.length > 0
      ? await db.payment.findMany({ where: { enrollmentId: { in: allEnrollmentIds } } })
      : [];

    const enrollmentPaymentsMap = new Map<string, (typeof allEnrollmentPayments)[number][]>();
    for (const p of allEnrollmentPayments) {
      if (p.enrollmentId) {
        const list = enrollmentPaymentsMap.get(p.enrollmentId);
        if (list) list.push(p);
        else enrollmentPaymentsMap.set(p.enrollmentId, [p]);
      }
    }
    ...
    for (const [, { student, levelName, subjectName }] of studentMap) {
      ...
      if (studentEnrollments.length > 0) {
        // ── Per-enrollment: calculate overdue for each enrollment independently ──
        for (const enrollment of studentEnrollments) {
          const ePayments = enrollmentPaymentsMap.get(enrollment.id) || [];
          const eDate = student.enrollmentDate instanceof Date
            ? student.enrollmentDate
            : new Date(student.enrollmentDate);

          const calcResult = calcClassroomOverdue(
          ...
          if (calcResult) {
            overdueStudents.push({
              ...
              levelName: enrollment.level?.nameAr || levelName,
              subjectName: enrollment.subject?.nameAr || subjectName,
              totalOverdue: calcResult.totalOverdue,
              monthsOverdue: calcResult.maxMonthsOverdue,
              hasPendingPayment: calcResult.hasPendingPayment,
              pendingPaymentId: calcResult.pendingPaymentId,
              nextDueDate: calcResult.nextDueDate,
              enrollmentId: enrollment.id,
              overduePayments: calcResult.overduePayments,
            });
          }
        }
      } else {
